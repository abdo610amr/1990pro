...
export function ProductsPage() {
  ...
  const filteredProducts = useMemo(() => {
    ...
    return (products ?? []).filter((product) => {
      ...
      const matchesStock =
        stockFilter === "all" || (product.stockStatus ?? "in_stock") === stockFilter;
  ...
  const handleBulkDelete = async () => {
    ...
    setSubmitting(true);
    try {
      await Promise.all(selectedIds.map((id) => productService.remove(id)));
      toast.success(`${selectedIds.length} products deleted`);
      setSelectedIds([]);
      setBulkDeleteOpen(false);
      await refetch();
    } catch (err) {
      toast.error(err instanceof Error ? err.message : "Failed to delete selected products");
    } finally {
      setSubmitting(false);
    }
  };
                      ...
                      onChange={(event) =>
                        setSelectedIds(
                          event.target.checked
                            ? Array.from(new Set([...selectedIds, ...filteredProducts.map((product) => product.id)]))
                            : selectedIds.filter((id) => !filteredProducts.some((product) => product.id === id))
                        )
                      }
      ...
      <AlertDialog open={bulkDeleteOpen} onOpenChange={setBulkDeleteOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This bulk action permanently removes every selected product from the catalog.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel disabled={submitting}>Cancel</AlertDialogCancel>
