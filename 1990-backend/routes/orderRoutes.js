...
router.post("/", uploadScreenshot, async (req, res) => {
  try {
    ...
    // Idempotency: if this client-generated reference has already produced an
    // order (network retry / double submit), return the existing one instead
    // of creating a duplicate (which would create a duplicate Odoo Sale Order).
    if (clientRef && String(clientRef).trim()) {
      const existingOrders = await readCollection("orders");
      const existing = existingOrders.find((o) => o.clientRef === clientRef);
      if (existing) {
        return res.json({
    ...
    const parsedShippingAddress = parseJsonField(shippingAddress, null);
    const newOrder = {
      id: orderId,
      ...
      // Odoo Sale Order reference — populated by the sync service once the SO
      // is created; null until then. Enables reconciliation both ways.
      odoo_so_id: null,
      odoo_so_name: null,
      ...
      items: itemsArray.map((item) => {
        ...
        const label = item.variant ?? item.size ?? item.selectedSize;
        const matchedVariant = matchedProduct
          ? getProductVariants(matchedProduct).find(
              (v) => v.label.toLowerCase() === String(label).toLowerCase()
            )
          : null;
        // SKU is the key Odoo resolves the Sale Order line by.
        const sku = item.sku ?? matchedVariant?.sku ?? null;
