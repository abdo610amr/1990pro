...
export function adaptBrands(brands: BackendBrand[]): Brand[] {
  return brands.map((brand) => ({
    ...
    story: brand.story ?? "",
    about: brand.about ?? "",
    description: (brand as { description?: string }).description || brand.about || brand.story || "",
    barcodePrefix: (brand as { barcodePrefix?: string }).barcodePrefix || "1990",
    commissionPercentage: (brand as { commissionPercentage?: number }).commissionPercentage || 0,
    status: (brand as { status?: string }).status || "active",
    productCount: (brand as { productCount?: number }).productCount || 0,
...
export function adaptProducts(
  ...
  return products.map((product) => {
    ...
    return {
      ...
      brandName: brand?.name ?? product.brandName ?? product.brand_name ?? "1990",
      brandLogo: resolveAssetUrl(product.brandLogo || brand?.logo || "/uploads/brand-logo.png"),
      type: product.productType === "originals" ? "originals" : "brand",
      colors:
        product.colors && product.colors.length
          ? product.colors
          : [{ name: "Default", hex: "#5E0F1D" }],
      sizes,
